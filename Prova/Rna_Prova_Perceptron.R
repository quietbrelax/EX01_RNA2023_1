source("Treinamento_Perceptron.R")
library('plot3D')
...
retlist = trainPerceptron(X, tags_1, eta, tol, mxepoc, 1)
...
for (i in seqi) {
  ...
  for (j in seqj) {
    cj <- cj + 1
    M[ci, cj] = yPerceptron(cbind(i, j), w_1, 1)
  }
}
...
retlist = trainPerceptron(K, tags_2, eta, tol, mxepoc, 1)
...
for (i in seqi_2) {
  ...
  for (j in seqj_2) {
    cj <- cj + 1
    N[ci, cj] = yPerceptron(cbind(i, j), w_2, 1)
  }
}
...
persp3D(seqi_2, seqj_2, N, counter = T, theta = 55, phi = 30, r = 40, d = 20)
...
for (i in seqi_Resul) {
  ...
  for (j in seqj_Resul) {
    cj <- cj + 1
    Resultado[ci, cj] = yPerceptron(cbind(i, j), w_1, 1) + yPerceptron(cbind(i, j), w_2, 1)
  }
}
